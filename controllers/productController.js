...
// ==================== PREPARE PRODUCT DATA - FULLY FIXED ====================
const prepareProductData = (body) => {
  console.log('\n📋 ========== PREPARING PRODUCT DATA ==========');
  const data = {};
  ...
  // ========== PRODUCT DETAILS - FIXED: These were missing ==========
  data.productTypes = safeString(body.productTypes);
  data.flavors = safeString(body.flavors);
  data.size = safeString(body.size);
  
  // ⚠️ CRITICAL FIX: These fields were not being saved
  ...
  // ========== MANUFACTURING - FIXED MAPPING ==========
  const manufacturerValue = safeString(body.manufacturer || body.manufacturerName);
  data.manufacturer = manufacturerValue;
  data.manufacturerName = manufacturerValue;
  
  const manufacturerAddressValue = safeString(body.manufacturerContact || body.manufacturerAddress);
  data.manufacturerContact = manufacturerAddressValue;
  data.manufacturerAddress = manufacturerAddressValue;
  ...
  // ========== PACKAGER - FIXED MAPPING ==========
  const packerContactString = safeString(body.packerContact);
  data.packerContact = packerContactString;
  ...
  // ========== MARKETER - FIXED MAPPING ==========
  const marketerString = safeString(body.marketerNameAddress);
  data.marketerNameAddress = marketerString;
  ...
  data.allergenInformation = safeString(body.allergenInformation);
  ...
  console.log('========== DATA PREPARATION COMPLETE ==========');
  console.log('🔍 Final data object keys:', Object.keys(data).length);
...
exports.createProduct = async (req, res) => {
  try {
    console.log('\n🆕 ==================== CREATING PRODUCT ====================');
    console.log('📥 Body Keys:', Object.keys(req.body).length);
    console.log('📥 Received materialTypes:', req.body.materialTypes);
    console.log('📥 Received customWeight:', req.body.customWeight);
    console.log('📥 Received customSizeInput:', req.body.customSizeInput);
    console.log('📸 Files:', req.files ? Object.keys(req.files) : 'None');
    ...
    console.log('📦 ID:', product._id);
    console.log('📝 Name:', product.name);
    console.log('🔧 Material Types:', product.materialTypes);
    console.log('⚖️ Custom Weight:', product.customWeight);
    console.log('📏 Custom Size Input:', product.customSizeInput);
    console.log('🏭 Manufacturer:', product.manufacturer);
    console.log('📦 Packager:', product.packagerName);
    console.log('🏪 Marketer:', product.marketerName);
    console.log('==================== END ====================\n');
...
exports.exportCSV = async (req, res) => {
  try {
    const products = await Product.find().populate('category', 'name').populate('subcategory', 'name').lean();
    let csv = 'ID,Name,Category,MRP,Price,Stock,Variations,Quality,Diet,State,Manufacturer,Packager,Marketer,MaterialTypes,CustomWeight,CustomSizeInput\n';
    products.forEach(p => {
      csv += `"${p._id}","${p.name}","${p.category?.name||''}",${p.oldPrice||0},${p.price||0},${p.stock||0},"${p.hasVariations?'Yes':'No'}","${p.quality}","${p.dietPreference}","${p.State||''}","${p.manufacturer||''}","${p.packagerName||''}","${p.marketerName||''}","${p.materialTypes||''}","${p.customWeight||''}","${p.customSizeInput||''}"\n`;
    });
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename=products-${Date.now()}.csv`);
    res.send(csv);
