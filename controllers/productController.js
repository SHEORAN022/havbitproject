...
exports.createProduct = async (req, res) => {
  try {
    ...
    if (req.files?.image?.[0]) {
      try {
        data.image = await uploadImage(req.files.image[0], 'products/main');
    ...
    if (req.files?.gallery) {
      ...
      const galleryUrls = await uploadMultipleImages(galleryFiles, 'products/gallery');
      data.gallery = galleryUrls.slice(0, 9);
      console.log(`✅ Gallery: ${data.gallery.length} images uploaded`);
    }
    ...
    if (data.hasVariations && data.variations.length > 0 && req.files?.variationImages) {
      ...
      for (let i = 0; i < varImages.length && i < data.variations.length; i++) {
        try {
          const url = await uploadImage(varImages[i], 'products/variations');
          if (url) {
            data.variations[i].image = url;
            console.log(`✅ Variation ${i + 1} image uploaded`);
          }
        } catch (error) {
          console.error(`❌ Variation ${i + 1} image upload failed:`, error.message);
        }
      }
    }
    ...
    if (!data.hasVariations && data.price <= 0) {
      return res.status(400).json({ 
        success: false, 
    ...
    console.log('📦 ID:', product._id);
    console.log('📝 Name:', product.name);
    console.log('🏪 Restaurant:', product.restaurantName);
    console.log('========== END ==========\n');
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    if (req.files?.image?.[0]) {
      ...
      // Upload new image
      updateData.image = await uploadImage(req.files.image[0], 'products/main');
    } else if (product.image) {
      updateData.image = product.image;
    }
...
exports.exportCSV = async (req, res) => {
  try {
    ...
    let csv = 'ID,Name,Category,Subcategory,Price,Stock,Quality,Diet,Restaurant,Brand,Manufacturer\n';
    
    products.forEach(p => {
      csv += `"${p._id}","${p.name}","${p.category?.name || ''}","${p.subcategory?.name || ''}",${p.price || 0},${p.stock || 0},"${p.quality || ''}","${p.dietPreference || ''}","${p.restaurantName || ''}","${p.brandName || ''}","${p.manufacturer || ''}"\n`;
    });
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (productData.name && productData.price) {
        try {
          ...
          if (productData.oldPrice) productData.oldPrice = parseFloat(productData.oldPrice);
